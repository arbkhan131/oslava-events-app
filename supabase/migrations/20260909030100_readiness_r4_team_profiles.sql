-- R4: staff provisioning/listing helpers and safer worker directory paging.
...
begin
  ...
  actor_role := private.current_actor_role();
  if actor_role is null or actor_role not in ('SUPER_ADMIN'::public.app_role, 'ADMIN'::public.app_role) then
    raise exception 'not authorized to view staff directory';
  end if;
  ...
  from public.profiles p
  where p.role in ('ADMIN'::public.app_role, 'CAPTAIN'::public.app_role, 'SUPERVISOR'::public.app_role)
    and (p_role_filter is null or p.role = p_role_filter)
    and (p_account_filter is null or p.account_status = p_account_filter)
    and (
    ...
    and (
      actor_role = 'SUPER_ADMIN'::public.app_role
      or p.role not in ('ADMIN'::public.app_role)
    )
  order by p.role, p.full_name, p.phone_e164
  limit safe_limit
  offset safe_offset;
end;
$$;
...
revoke all on function public.provision_staff_profile(
  uuid,
  public.app_role,
  text,
  text,
...
revoke all on function public.staff_directory(
  text,
  public.app_role,
  public.account_status,
  integer,
...
grant execute on function public.staff_directory(
  text,
  public.app_role,
  public.account_status,
  integer,
